...
test("renders complete social preview metadata", async () => {
  ...
  assert.match(html, /<meta[^>]+property="og:image"[^>]+content="https:\/\/amana-aug-2026\.truechristian\.church\/assets\/social\/true-christian-share-600-v3\.png"/i);
  assert.match(html, /<meta[^>]+property="og:image:secure_url"[^>]+content="https:\/\/amana-aug-2026\.truechristian\.church\/assets\/social\/true-christian-share-600-v3\.png"/i);
  assert.match(html, /<meta[^>]+property="og:image:width"[^>]+content="600"/i);
  assert.match(html, /<meta[^>]+property="og:image:height"[^>]+content="600"/i);
  ...
  assert.match(html, /<link[^>]+rel="image_src"[^>]+href="https:\/\/amana-aug-2026\.truechristian\.church\/assets\/social\/true-christian-share-600-v3\.png"/i);
...
test("ships the social preview image as a PNG", async () => {
  const image = await readFile(new URL("../public/assets/social/true-christian-share-600-v3.png", import.meta.url));
